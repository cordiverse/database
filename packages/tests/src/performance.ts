...
function PerformanceTests(database: Database) {
  ...
  it('bulk upsert nested rows', async (ctx) => {
    ...
    await benchmark(ctx.task.name, 5, () => database.upsert('perfNested', payload, ['id']))
  ...
  it('bulk upsert direct fields', async (ctx) => {
    ...
    await benchmark(ctx.task.name, 8, () => database.upsert('perf', payload, ['id']))
  ...
  it('bulk upsert expression fields', async (ctx) => {
    await benchmark(ctx.task.name, 8, () => database.upsert('perf', row => {
  ...
  it('bulk upsert dotted fields', async (ctx) => {
    const payload = new Array(200).fill(0).map((_, i) => ({
      id: i + 1,
      'meta.label': `dotted-${i}`,
      'meta.score': i * 5,
    ...
    await benchmark(ctx.task.name, 6, () => database.upsert('perfNested', payload, ['id']))
  ...
  it('repeated get single flat row', async (ctx) => {
    const row = await benchmark(ctx.task.name, 400, (index) => {
  ...
  it('repeated get single nested row', async (ctx) => {
    const row = await benchmark(ctx.task.name, 300, (index) => {
  ...
  it('repeated set single flat row', async (ctx) => {
    await benchmark(ctx.task.name, 300, (index) => {
  ...
  it('repeated set single nested row', async (ctx) => {
    await benchmark(ctx.task.name, 200, (index) => {
  ...
  it('repeated upsert single flat row', async (ctx) => {
    await benchmark(ctx.task.name, 300, (index) => {
  ...
  it('repeated upsert single nested dotted row', async (ctx) => {
    await benchmark(ctx.task.name, 200, (index) => {
  ...
  it('repeated create and remove single flat row', async (ctx) => {
    const created = await benchmark(ctx.task.name, 150, async (index) => {
